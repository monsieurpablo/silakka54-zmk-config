# build-lily58.ps1
# Optimized ZMK builder for Lily58 (sequential builds with incremental compilation)
...
Write-Host "    ZMK Lily58 Builder (Optimized)" -ForegroundColor Yellow
...
docker run --rm `
    ...
    sh -c "cd /zmk && west build -d build/left -b nice_nano -s app/ -- -DSHIELD=lily58_left -DZMK_CONFIG=/config && cp build/left/zephyr/zmk.uf2 /output/lily58_left.uf2"
...
docker run --rm `
    ...
    -v "${OUTPUT_PATH}:/output" `
    zmkfirmware/zmk-build-arm:3.5-branch `
    sh -c "cd /zmk && west build -d build/right -b nice_nano -s app/ -- -DSHIELD=lily58_right -DZMK_CONFIG=/config && cp build/right/zephyr/zmk.uf2 /output/lily58_right.uf2"
